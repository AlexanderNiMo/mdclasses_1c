...
Функция РедакцияКонфигурации() Экспорт
	...
	Результат = "";
	ВерсияКонфигурации = ПРГ_ОбщегоНазначенияСервер.ПолучитьВерсиюКонфигурации(); //Прогтехника
	...
	Если ПустаяСтрока(Результат) Тогда
		Результат = ПРГ_ОбщегоНазначенияСервер.ПолучитьВерсиюКонфигурации(); //Прогтехника
	КонецЕсли;
...
Функция СсылкаСуществует(ПроверяемаяСсылка) Экспорт
	...
	//Прогтехника с
	Если ПроверяемаяСсылка = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	//Прогтехника с
...
&НаСервере
&После("МакетСуществует")
Функция А_МакетСуществует(ПолноеИмяМакета)
